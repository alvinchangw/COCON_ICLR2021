...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def generate(
        ...
        if do_cocon:
            output = self._generate_cocon_no_beam_search(
                input_ids,
                cur_len,
                max_length,
                do_sample,
                ...
                cocon_block,
                cocon_context_inputs,
                cocon_history_inputs,
                cocon_after_block_ind, # output_after_block_ind arg for model,
                transform_h_after_layernorm,
                termination_token_id,
                use_only_last_cocon_output_for_ar,
                context_attn_bias,
            )
        else:
            if num_beams > 1:
    ...
    def _generate_cocon_no_beam_search(
        self,
        input_ids, # TIS input, default shape is [N, 0]
        cur_len,
        max_length,
        do_sample,
        temperature,
        ...
        with torch.no_grad():
            if type(cocon_context_inputs) != list:
                ...
                # iterate and concatentate multiple context_seq_hidden_states
                context_seq_hidden_state_list = []
                if type(cocon_context_inputs) == list:
                    context_seq_len_list = []
                else:
                    context_seq_len_list = None
                for cocon_context_input in cocon_context_inputs:
                    ...
                    context_seq_hidden_state_list.append(context_seq_hidden_state)
        ...
        while cur_len < max_length:
            # cocon code start
            # Compute input hidden state
            if input_ids.shape[-1] == 0:
                hidden_states = None
            else:
                # concat input_ids with history_seq ids
                hist_plus_input_ids = torch.cat([cocon_history_inputs, input_ids], dim=1)

                if transform_h_after_layernorm:
                    hist_plus_input_hidden_states = self(hist_plus_input_ids, output_after_block_ind=cocon_after_block_ind, return_point='next_block_ln_1')[0]  # [N, L, C]
                else:
                    hist_plus_input_hidden_states = self(hist_plus_input_ids, output_after_block_ind=cocon_after_block_ind)[0]  # [N, L, C]

                hidden_states = hist_plus_input_hidden_states[:, cocon_history_inputs.shape[1]:]

            if context_attn_bias == 0 :
                cocon_transformed_hidden_states = cocon_block(hidden_states, context_seq=context_seq_hidden_states, history_seq=history_seq_hidden_states, include_sos_output=True) # [N, L, C]
            else:
                cocon_transformed_hidden_states = cocon_block(hidden_states, context_seq=context_seq_hidden_states, history_seq=history_seq_hidden_states, include_sos_output=True, context_attn_bias=context_attn_bias, context_seq_len_list=context_seq_len_list) # [N, L, C]

            if use_only_last_cocon_output_for_ar:
                if hidden_states is not None:
                    hist_plus_cocon_hidden_states = torch.cat([hist_plus_input_hidden_states[:, :-1], cocon_transformed_hidden_states[:, -1:]], dim=1)
                else:
                    hist_plus_cocon_hidden_states = torch.cat([history_seq_hidden_states[:, :-1], cocon_transformed_hidden_states[:, -1:]], dim=1)

            else:
                hist_plus_cocon_hidden_states = torch.cat([history_seq_hidden_states[:, :-1], cocon_transformed_hidden_states], dim=1)

            if transform_h_after_layernorm:
                outputs = self(input_hidden_state=hist_plus_cocon_hidden_states, input_before_block_ind=cocon_after_block_ind+1, input_point='current_block_ln_1')
            else:
                outputs = self(input_hidden_state=hist_plus_cocon_hidden_states, input_before_block_ind=cocon_after_block_ind+1)
            # cocon code end
            ...
            if termination_token_id is not None and next_token == termination_token_id:
                break
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            outputs = self(**model_inputs) # add adaIN_vector here for adaIN gen
